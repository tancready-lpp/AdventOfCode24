...
and so only some of the ordering rules apply - within each update, the ordering rules 
...
class Puzzle5:
    ...
    def part1(self):
        middles = []
        rules = self.get_input()[0]
        updates = self.get_input()[1]
        for update in updates:
            positions = {value:i for i, value in enumerate(update)}
            for rule in rules:
                if rule[0] in positions and rule[1] in positions:
                    if positions[rule[0]] >= positions[rule[1]]: # this is what we DO NOT want! 
                        break # invalid update, skip to the next one
            else:
                # only runs if the for-loop didn’t break → the update is valid
...
datafile = "/Users/tancredi/Desktop/python/AdventOfCode/2024/data_for_puzzles/puzzle5.txt"
po = Puzzle5(datafile)
# print(po.get_input()[1])
print(po.part1())
